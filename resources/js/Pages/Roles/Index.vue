            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-b-lg">
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="space-y-4">
                            <div class="flex px-4 py-6 border border-gray-200" v-for="role in roles.data">
                                <div class="md:w-3/4">
                                    <b>{{ role.title }}</b>
                                    <br>
                                </div>
                                <div class="md:w-1/4">
                                    <BtnDeleteForm :routeName="routeName" :subjectId="role.id" >{{ t('delete') }}</BtnDeleteForm>
                                    <Link :href="route(`${routeName}.edit`, {role})" class="edit-button hover:bg-blue-700">{{ $t('edit') }}</Link>
                                    <Link :href="route(`${routeName}.show`, {role})" class="edit-button hover:bg-blue-700 ml-2">{{ $t('details') }}</Link>
                                </div>
                            </div>
                        </div>
                        <div v-if="(!roles.total)" class="text-sm text-center mt-4 mb-4">{{ t('no_results') }}</div>
                    </div>
                </div>
